...
export default function Side({ children }: SideProps) {
    ...
    const handleLogout = () => {
        ...
        ClearItemCache();
        ClearBorrowCache();
        signOut({ callbackUrl:'/pages/admin/login' });//{ callbackUrl:'/pages/admin/login' }
    };
                        ...
                        <div className="flex justify-around w-full bg-blue-900 text-slate-100" onClick={() => router.push('/pages/admin/profile')} >
                            <div className="flex w-full items-center mb-5 cursor-pointer m-5">
                                <Avatar className="w-12 h-12 rounded-full overflow-hidden mb-2 mr-4">
                                    <AvatarImage className="w-full h-full object-cover" src={'http://172.20.48.135:9000/images' + '/profile/' + session.user.image} />
                                </Avatar>
