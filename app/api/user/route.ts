...
export async function PUT(req: Request) {
  ...
  try {
    const { id, name, lastname, tel, email, username, password, oldPassword } =
      await req.json();
    let hashedPassword;
    if (oldPassword) {
    ...
    return NextResponse.json({
      error: "someting went worng at " + url.href + err,
      status: 500,
    });
  }
}
...
export async function POST(req: Request) {
  const url = new URL(req.url);
  try {
    const { name, lastname, email, username, password, gender, tel, role } =
      await req.json();
    ...
    console.error(error);
    return NextResponse.json({
      error: "someting went worng at " + url.href + " " + error,
      status: 500,
    });
  }
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      error: "someting went worng at " + url.href,
      status: 500,
    });
  }
}
